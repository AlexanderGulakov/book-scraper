...
# Мова видання, названа прямо в оголошенні. Українське оголошення пише саме
# так: «польською», «в'єтнамською», «німецькою». Список явний, а не регулярка
# на «-ською»: під неї підпадають «міською», «київською» й інші прикметники,
# і ціна помилки тут — мовчки викинута потрібна книжка.
FOREIGN_LANGUAGE_MARKERS = [
    "польською", "польскою", "російською", "білоруською", "німецькою",
    "французькою", "іспанською", "італійською", "португальською",
    "нідерландською", "голландською", "шведською", "норвезькою", "данською",
    "фінською", "чеською", "словацькою", "словенською", "хорватською",
    "сербською", "болгарською", "румунською", "угорською", "грецькою",
    "турецькою", "литовською", "латиською", "латвійською", "естонською",
    "грузинською", "вірменською", "казахською", "узбецькою", "азербайджанською",
    "китайською", "японською", "корейською", "арабською", "івритом",
    "в'єтнамською", "в'єтнамскою", "в'єтнамській", "тайською", "гінді",
    "мовою оригіналу",
]
...
def norm_apostrophes(text: str) -> str:
    """Апострофи на OLX пишуть чотирма різними символами. Без зведення до
    одного «в'єтнамською» і «вʼєтнамською» — два різні рядки."""
    out = text or ""
    for ch in "\u2019\u02bc\u0060\u00b4\u02b9":
        out = out.replace(ch, "'")
    return out
...
def foreign_language(text: str, markers: Iterable[str] = ()) -> str | None:
    """Назва чужої мови в тексті, якщо вона там є.

    Українську сюди не додаємо ніколи: «українською» — це якраз те, що треба.
    """
    t = norm_apostrophes(text or "").casefold()
    for m in (markers or FOREIGN_LANGUAGE_MARKERS):
        if m and norm_apostrophes(m).casefold() in t:
            return m
    return None
...
def decide(*, title: str, price: float | None, watch: dict[str, Any],
    ...
    # 3а. Чужа мова, названа прямо («в'єтнамською», «польською»). Окремо від
    #     англійської, бо там розпізнавання інше — за часткою латиниці.
    #     Привід: під «Напівкровним принцом» найдешевшим показали те саме
    #     видання в'єтнамською за 70 грн.
    if _opt(entry, watch, "drop_foreign", default=True):
        lang = foreign_language(hay, _opt(entry, watch, "foreign_markers",
                                          default=FOREIGN_LANGUAGE_MARKERS) or ())
        if lang:
            return Decision("skip", reason=f"видання «{lang}»")
